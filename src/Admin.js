...
const Admin = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('https://sneakerhub-vsiq.onrender.com/api/admin/upload', formData, {
        headers: { 
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [ord, prod] = await Promise.all([
        axios.get('https://sneakerhub-vsiq.onrender.com/api/orders', { headers }),
        axios.get('https://sneakerhub-vsiq.onrender.com/api/products')
      ]);
  ...
  const handleUpdateOrderStatus = async (id, newStatus) => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`https://sneakerhub-vsiq.onrender.com/api/orders/${id}/status`, { status: newStatus }, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  const handleAddOrUpdateProduct = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const headers = { 'Authorization': `Bearer ${token}` };
      if (editingId) { await axios.put(`https://sneakerhub-vsiq.onrender.com/api/admin/products/${editingId}`, data, { headers }); }
      else { await axios.post('https://sneakerhub-vsiq.onrender.com/api/admin/products', data, { headers }); }
        ...
        {filteredProducts.map(p => (
          <div key={p._id} style={styles.card} className="admin-card">
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button onClick={async () => { if(window.confirm("Удалить?")) { await axios.delete(`https://sneakerhub-vsiq.onrender.com/api/admin/products/${p._id}`, { headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` } }); fetchData(); } }} style={{ flex: 1, background: '#ff174420', color: '#ff1744', border: 'none', padding: '8px', borderRadius: '6px', cursor: 'pointer' }}>УДАЛИТЬ</button>
            </div>
          </div>
        ))}
      </div>
